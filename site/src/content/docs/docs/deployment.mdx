...
The `release-desktop.yml` workflow builds Windows and macOS packages for version tags and uploads them to the matching GitHub Release. Local packages can be produced from `desktop/` with `npm run tauri build`. macOS artifacts are currently unsigned/notarized, so release notes must keep the trust limitation visible.
