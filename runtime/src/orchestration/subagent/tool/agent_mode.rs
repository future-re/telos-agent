...
use crate::knowledge::tasks::{Task, TaskStatus};
use crate::orchestration::subagent::tool::SubagentTool;
use crate::orchestration::subagent::{AgentDefinition, create_subagent_worktree};
use crate::tools::api::{ToolContext, ToolOutput, ToolProgress, ToolRegistry};
...
async fn run_child_agent(
    provider: std::sync::Arc<dyn crate::model::provider::ModelProvider + Send + Sync>,
    tools: ToolRegistry,
    ...
    let turn_result = {
        let mut handle = runtime.start_turn(&session, prompt)?;
    ...
    let final_text = turn_result.final_message.text_content();

    let tool_call_count = messages.iter().flat_map(|m| m.tool_calls()).count();
