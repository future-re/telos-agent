...
use crate::model::provider::{CompletionRequest, ModelProvider};
...
async fn execute_lens(shared: &ForkShared, lens: &ForkLens) -> Option<ForkResult> {
    ...
    let request = CompletionRequest {
        system_prompt_blocks: vec![], // system message already in fork_messages
...
fn role_str(role: &crate::model::message::Role) -> &str {
    match role {
        crate::model::message::Role::System => "system",
        crate::model::message::Role::User => "user",
        crate::model::message::Role::Assistant => "assistant",
        crate::model::message::Role::Tool => "tool",
    }
}
