...
) -> Result<(SessionInfo, Conversation, RuntimeState), AgentError> {
    config.validate()?;
    ...
    let metrics = metadata
        ...
        .map(|metadata| {
            SessionMetrics::with_values(
                ...
                metadata.total_prompt_cache_hit_tokens,
                metadata.total_prompt_cache_miss_tokens,
                metadata.total_tool_calls,
                metadata.total_tool_errors,
        ...
        .unwrap_or_else(SessionMetrics::new);
    let read_file_state = std::sync::Arc::new(tokio::sync::Mutex::new(
        metadata.map(|metadata| metadata.read_file_state).unwrap_or_default(),
    ));

    Ok((
        SessionInfo::with_id(session_id_str, config, next_turn_id, event_channel),
        Conversation::with_messages(messages),
        RuntimeState::with_values(metrics, read_file_state, 0),
    ))
}
