...
**Architecture:** 根 `Cargo.toml` 添加 `[workspace]`，`cli/` 重命名为 `cli/`。CLI 内部新增 `tui/` 模块树，通过 `tokio::sync::mpsc` 将 `AgentSession::run_turn_stream` 的 `TurnEvent` 流转发给 ratatui 的事件循环，`ApprovalHandler` 通过 `oneshot` 桥接实现交互式审批。现有 `telos "prompt"` one-shot 模式和 `telos completion` 保持不变。
...
将 README.md 中所有 `cli/` 路径替换为 `cli/`。具体：
...
- Rename cli/ to cli/ for cleaner naming
- Update README paths
